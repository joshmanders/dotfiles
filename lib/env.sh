...
# Check if config.sh exists, run setup wizard if not
#
# Usage:
#   ensure_config
#
# Parses config.sh.example and prompts for each variable:
#   - Empty values (VAR=) are required
#   - Values with defaults (VAR="default") show the default, enter to accept
#   - Comment above each export is used as the prompt
#
...
ensure_config() {
    ...
    # Parse example file and collect values
    local last_comment=""
...
"
    ...
    while IFS= read -r line; do
        # Capture comments (potential prompts)
        if [[ "$line" =~ ^#[[:space:]]*(.+)$ ]]; then
        ...
        if [[ "$line" =~ ^export[[:space:]]+([A-Z_]+)=(.*)$ ]]; then
            ...
            # Strip quotes from default
            default_value="${default_value#\"}"
            default_value="${default_value%\"}"

            local prompt="${last_comment:-$var_name}"
            local value=""

            if [[ -n "$default_value" ]]; then
                # Has default - show it, enter to accept
